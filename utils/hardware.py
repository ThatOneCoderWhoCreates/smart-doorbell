...
class HardwareInterface:
    def __init__(self, relay_pin=17, pir_pin=27):
        ...
        if IS_RPI:
            ...
            GPIO.add_event_detect(self.pir_pin, GPIO.RISING, callback=self._pir_triggered, bouncetime=2000)
            log("Initialized Real GPIO (Raspberry Pi mode)")
        else:
            log("Initialized Mock GPIO (PC testing mode)")

    def _pir_triggered(self, channel):
    ...
    def unlock_door(self, duration=3):
        """Activates the relay to unlock the door for a few seconds"""
        if IS_RPI:
            ...
            threading.Thread(target=lock_again).start()
        else:
            log(f"Mock GPIO: DOOR UNLOCKED for {duration} seconds")
            
    def lock_door(self):
        """Immediately activates the relay to lock the door."""
        if IS_RPI:
    ...
    def mock_pir_trigger(self):
        """Artificially triggers the PIR for testing on PC"""
        if not IS_RPI:
            self._pir_triggered(self.pir_pin)
